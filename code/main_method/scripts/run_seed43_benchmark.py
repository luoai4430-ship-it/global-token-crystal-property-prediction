#!/usr/bin/env python3
"""Paired Global/TrueToken seed-43 benchmark for the ten final tasks."""
...
def verify_checkpoint_reload(
    ...
    def predict(model: nn.Module) -> torch.Tensor:
        # Graph encoders may update batch attributes in place, so every
        # forward receives independent graph and token tensors.
        with torch.no_grad(), torch.autocast(
            device_type=device.type,
            dtype=TRAINING_DTYPE,
            enabled=device.type == "cuda",
            ...
            return model(
                first.graph.clone(),
                first.text_tokens.clone(),
                first.attention_mask.clone(),
                input_ids=first.input_ids.clone(),
            )["final_prediction"].detach()

    reference_predictions = [predict(reference_model) for _ in range(3)]
    reloaded_predictions = [predict(restored) for _ in range(3)]
    all_predictions = reference_predictions + reloaded_predictions
    if not all(bool(torch.isfinite(values).all()) for values in all_predictions):
        raise AssertionError("checkpoint reload produced non-finite predictions")

    def maximum_pairwise_difference(predictions: list[torch.Tensor]) -> float:
        return max(
    ...
    same_model_max_abs = max(
        maximum_pairwise_difference(reference_predictions),
        maximum_pairwise_difference(reloaded_predictions),
    )
    reload_max_abs = max(
        float((reference - reloaded).abs().max().cpu())
        for reference in reference_predictions
        for reloaded in reloaded_predictions
    )
    bf16_floor = float(4 * torch.finfo(torch.bfloat16).eps)
    reload_tolerance = max(bf16_floor, 2.0 * same_model_max_abs + bf16_floor)
    if reload_max_abs > reload_tolerance:
        raise AssertionError(
            "checkpoint reload predictions exceed repeated-forward BF16 "
            f"tolerance: reload={reload_max_abs} same_model={same_model_max_abs} "
            f"tolerance={reload_tolerance}"
        )
    return {
        "checkpoint_reload_state_exact": True,
        "checkpoint_reload_max_abs": reload_max_abs,
        "checkpoint_reload_same_model_max_abs": same_model_max_abs,
        "checkpoint_reload_repeated_forward_count": 3,
        "checkpoint_reload_rtol": 0.0,
        "checkpoint_reload_atol": reload_tolerance,
    }
...
def recover_valid(args: argparse.Namespace) -> None:
    ...
    recovered_valid_mae_abs_diff = abs(
        float(checkpoint["best_valid_mae"]) - valid_metrics["mae"]
    )
    recovery_valid_mae_tolerance = 5e-4
    if recovered_valid_mae_abs_diff > recovery_valid_mae_tolerance:
        raise AssertionError(
            "recovered Valid MAE differs from saved best checkpoint: "
            f"diff={recovered_valid_mae_abs_diff} "
            f"tolerance={recovery_valid_mae_tolerance}"
        )
    if valid_metrics["special_token_attention_max"] != 0.0:
        raise AssertionError("special tokens received attention")
    if valid_metrics["padding_attention_max"] != 0.0:
    ...
    summary = {
        ...
        **reload_diagnostics,
        ...
        "checkpoint_saved_valid_mae": float(checkpoint["best_valid_mae"]),
        "recovered_valid_mae_abs_diff": recovered_valid_mae_abs_diff,
        "recovery_valid_mae_tolerance": recovery_valid_mae_tolerance,
    }
    atomic_json(args.output_dir / "summary.json", summary)
    print(json.dumps(summary, indent=2, sort_keys=True), flush=True)
